...
{
	this->N = N;
	WS = new float[N]; memset(WS, 0, N * sizeof(float));
	HUn = new float[N+1]; memset(HUn, 0, (N+1) * sizeof(float));
	HUUn = new float[N]; memset(HUUn, 0, N * sizeof(float));
}
...
void flux_staggered::ComputeFlux(const State& S0, const unsigned int& OrderReconstruction)
{
	if (OrderReconstruction != 1 && OrderReconstruction != 2) {
		std::cout << "Error: Order of reconstruction not implemented for the conservative staggered scheme" << std::endl;
		exit(1);
	}
	...
	for (unsigned int i = 0; i < (N + 1); i++) {
		...
		if (i == 0) { HUn[i] = S0.Un[i] * S0.Hn[i]; }
		else if (i == N) { HUn[i] = S0.Un[i]  * S0.Hn[i-1]; }
			...
			HUn[i] = S0.Un[i] * upwind(S0.Un[i] , S0.Hn[i-2], S0.Hn[i-1], S0.Hn[i], S0.Hn[i+1]);
		}
	}	
...
{
	if (V >= 0) return (Q2 + 0.5*minmod(thetaRC*(Q3 - Q2), 0.5*(Q3 - Q1),thetaRC*(Q2 - Q1)));
	else return (Q3 - 0.5*minmod(thetaRC * (Q4 - Q3), 0.5*(Q4 - Q2),thetaRC * (Q3 - Q2)));
}
...
flux::flux(unsigned int N)
{
	this->N = N;
	WS = new float[N]; memset(WS, 0, N * sizeof(float));
	Hn = new float[N]; memset(Hn, 0, N * sizeof(float));
	Qn = new float[N]; memset(Qn, 0, N * sizeof(float));
	HL = new float[N-1]; memset(HL, 0, (N-1) * sizeof(float));
	HR = new float[N-1]; memset(HR, 0, (N-1) * sizeof(float));
	QL = new float[N-1]; memset(QL, 0, (N-1) * sizeof(float));
	QR = new float[N-1]; memset(QR, 0, (N-1) * sizeof(float));
	FluxH = new float[N-1]; memset(FluxH, 0, (N-1) * sizeof(float));
	FluxU = new float[N-1]; memset(FluxU, 0, (N-1) * sizeof(float));
}
...
void flux::ComputeFlux(const State& S0, const scheme& Scheme, const unsigned int& OrderReconstruction)
{
	// Compute Qn = Hn*Un
	#pragma omp parallel for
	for(unsigned int i=0; i<N; i++){
...
void Recontruction_Order2(float* Hn, float* Hl, float *Hr, unsigned int N)
{
	...
	for(unsigned int i=1; i<N-1; i++){
		Hx[i] = minmod(thetaRC*(Hn[i]-Hn[i-1]), 0.5*(Hn[i+1]-Hn[i-1]) ,thetaRC*(Hn[i+1]-Hn[i]));
	}
...
void Recontruction_Order3(float* Hn, float* Hl, float *Hr, unsigned int N)
{
	...
	for (unsigned int i = 2; i < N - 3; i++) {
		...
		Hl[i] = Hn[i] + 0.5*std::max(0.0f, min3(2.0f,2*rL[i], bL))*(Hn[i] - Hn[i - 1]) ;
		Hr[i] = Hn[i + 1] - 0.5*std::max(0.0f, min3(2.0f, 2 * rR[i+1], bR))*(Hn[i+2] - Hn[i+1]);
	}
	Hl[0]= Hn[0];  Hr[0]= Hn[1];
	Hl[1] = Hn[1];  Hr[1] = Hn[2];

	Hl[N-2] = Hn[N-2];  Hr[N - 2] = Hn[N-1];
	Hl[N-3] = Hn[N-3];  Hr[N-3] = Hn[N-2];
...
void Recontruction_Order5(float* Hn, float* Hl, float *Hr, unsigned int N)
{
	...
	for (unsigned int i = 2; i < N - 3; i++) {
		...
		bL = (-2.0 / rL[i-1] + 11.0 + 24.0*rL[i] - 3.0*rL[i] * rL[i+1]) / 30.0;
		bR = (-2.0 / rR[i +2] + 11.0 + 24.0*rR[i+1] - 3.0*rR[i] * rR[i + 1]) / 30.0;

		Hl[i] = Hn[i] + 0.5*std::max(0.0f, min3(2.0,2*rL[i], bL))*(Hn[i] - Hn[i - 1]) ;
		Hr[i] = Hn[i + 1] - 0.5*std::max(0.0f, min3(2.0, 2 * rR[i+1], bR))*(Hn[i+2] - Hn[i+1]);
	}
	Hl[0]= Hn[0];  Hr[0]= Hn[1];
	Hl[1] = Hn[1];  Hr[1] = Hn[2];
...
void Flux_CentralUpwind(float* Hl, float *Hr, float* Ql, float *Qr, float *FluxH, float *FluxQ, unsigned int N)
{
	...
	for(unsigned int i=0; i<N-1; i++){
		...
		FluxH[i] = (aR*FhL - aL*FhR)/(aR - aL) + (aR*aL*(Hr[i] - Hl[i]))/(aR - aL);
	    FluxQ[i] = (aR*FuL - aL*FuR)/(aR - aL) + (aR*aL*(Qr[i] - Ql[i]))/(aR - aL);
	}
...
void Flux_HLL(float* Hl, float *Hr, float* Ql, float *Qr, float *FluxH, float *FluxQ, unsigned int N)
{
	...
	for(unsigned int i=0; i<N-1; i++){
		...
		// Left state
    	hL = Hl[i]; uL = Ql[i]/Hl[i];
    	FhL = hL*uL; FuL = hL*uL*uL + G*hL*hL/2;
        
    	// Right state
		hR = Hr[i]; uR = Qr[i]/Hr[i];
		FhR = hR*uR; FuR = hR*uR*uR + G*hR*hR/2;
		
		// Compute h*
    	h0 = 0.5*(hL + hR) -0.25*(uR - uL)*(hL + hR)/(sqrt(G*hL) + sqrt(G*hR));
    	
    	// Compute QL, QR
	    if (h0>hL) { qL = sqrt(0.5*(h0+hL)*h0/(hL*hL));}
	    else { qL = 1; }
	    
	    if (h0>hR) { qR = sqrt(0.5*(h0+hR)*h0/(hR*hR)); }
	    else { qR = 1;} 
	    ...
	    if (0<sL){
	    	...
	    	FluxH[i] = (sR*FhL-sL*FhR + (sL*sR)*(Hr[i]-Hl[i]))/(sR-sL);
	    	FluxQ[i] = (sR*FuL-sL*FuR + (sL*sR)*(Qr[i]-Ql[i]))/(sR-sL);
		}
	}
...
void Flux_HLLC(float* Hl, float *Hr, float* Ql, float *Qr, float *FluxH, float *FluxQ, unsigned int N) 
{
    ...
    for (unsigned int i = 0; i < N - 1; i++) {
		...
		// Compute h*
    	h0 = 0.5*(hL + hR) -0.25*(uR - uL)*(hL + hR)/(sqrt(G*hL) + sqrt(G*hR));
		u0 = 0.5*(uL + uR) - (hR - hL)*(sqrt(G*hL) + sqrt(G*hR))/(hL + hR);
		...
		s0 = (sL * hR * (uR - sR) - sR * hL * (uL - sL)) / (hR * (uR - sR) - hL * (uL - sL));
