...
Input::Input(std::string nametest) {
  ...
  resultsFolder =
      resultsFolder.substr(0, resultsFolder.find(".")); // remove file extension
...
void Input::LOGOUT() {
  ...
  std::cout << std::left << std::setw(25) << "frequency [Hz]" << std::setw(25)
            << "amplitude [m]" << std::setw(25) << "phase [rad]" << std::endl;
