...
State::State(const Input& In)
{
    ...
    depth = In.WaterDepth;
    if(In.Scheme == scheme::conservative_staggered) Grid = grid::staggered;
    else Grid = grid::collocated;

    // allocate memory
    Eta = new float[N]; memset(Eta, 0, N * sizeof(float));
    Hn = new float[N]; memset(Hn, 0, N * sizeof(float));
    Zn = new float[N]; memset(Zn, 0, N * sizeof(float));

    if (Grid == grid::staggered)
    {
        Un = new float[N+1]; memset(Un, 0, (N+1) * sizeof(float));
        Pn = new float[N+1]; memset(Pn, 0, (N+1) * sizeof(float));
    }
    else
    {
        Un = new float[N]; memset(Un, 0, N * sizeof(float));
        Pn = new float[N]; memset(Pn, 0, N * sizeof(float));
    }
...
void PrintVariables(State& S0, State& S2, const Input &In, const float& t1, const float& dt1, unsigned int& Kprint, float& Tprint)
{
	...
	while (t1 <= Tprint && t1 + dt1 >= Tprint && Tprint <= In.Time) {
		...
		linear_interpolation(S0.Eta, S2.Eta, N, t1, dt1, Tprint);	// The intrpolated value is saved in Eta - wont be used later
																			// Memory recycling
        std::string namefile = In.ResFolder + "/FreeSurface/H_" + intToStringWithLeadingZeros(Kprint, 6) + ".bin";
...
{
	while (t1 <= Tgauges && t1 + dt1 >= Tgauges && (Tgauges <= In.Time || std::abs(Tgauges - In.Time) < 5e-5)) {

		for(unsigned int k=0; k<In.IndexGauges.size(); k++)
		{
			int i = In.IndexGauges[k];
            float value = linear_interpolation(S0.Eta[i], S2.Eta[i], t1, dt1, Tgauges);
...
void PrintTimeSeries(std::vector<float> *Hgauges, const Input &In)
{
    for(unsigned int k=0; k<In.IndexGauges.size(); k++)
    {
        std::string namefile = In.ResFolder + "/TimeSeries/Gauge_" + std::to_string(k) + ".bin";
...
std::string intToStringWithLeadingZeros(int number, int totalLength) {
    return std::string(totalLength - std::to_string(number).length(), '0') + std::to_string(number);
}
