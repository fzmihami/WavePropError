...
int main(int argc, char** argv)
{
    ...
    S0 = new State(In);
    S1 = new State(In);
    if (In.OrderTime >= 2) S2 = new State(In);
    if (In.OrderTime >= 3) S3 = new State(In);
    ...
    while(t1 <= In.Time)
    {
        ...
        if(S0->Grid == grid::staggered)
        {
            ...
            {
                SolveEquationStaggered_step1(*S0, *S1, In, *Fstaggered, Disp, WM, dt1);
                SolveEquationStaggered_step2(*S0, *S1, *S2, In, *Fstaggered, Disp, WM, dt1);
            }
            ...
            else if(In.OrderTime == 3)
            {
                SolveEquationStaggered_step1(*S0, *S1, In, *Fstaggered, Disp, WM, dt1);
                SolveEquationStaggered_step2(*S0, *S1, *S2, In, *Fstaggered, Disp, WM, dt1);
                SolveEquationStaggered_step3(*S0, *S2, *S3, In, *Fstaggered, Disp, WM, dt1);
            }
            ...
            if(In.OrderTime == 1)
            {
                SolveEquationCollocated_step1(*S0, *S1, In, *Fcollocated, Disp, WM, dt1);
            }

            else if(In.OrderTime == 2)
            {
                SolveEquationCollocated_step1(*S0, *S1, In, *Fcollocated, Disp, WM, dt1);
                SolveEquationCollocated_step2(*S0, *S1, *S2, In, *Fcollocated, Disp, WM, dt1);
            }
            ...
            else if(In.OrderTime == 3)
            {
                SolveEquationCollocated_step1(*S0, *S1, In, *Fcollocated, Disp, WM, dt1);
                SolveEquationCollocated_step2(*S0, *S1, *S2, In, *Fcollocated, Disp, WM, dt1);
                SolveEquationCollocated_step3(*S0, *S2, *S3, In, *Fcollocated, Disp, WM, dt1);
            }
        ...
        if(In.OrderTime == 1) {
            if(In.IndexGauges.size() != 0) RecordTimeSeries(*S0, *S1, In, Hgauge, t1, dt1, Kgauges, Tgauges);
            PrintVariables(*S0, *S1, In, t1, dt1, Kprint, Tprint);
            S0->Update(*S1);
        }

        else if(In.OrderTime == 2) {
            if(In.IndexGauges.size() != 0) RecordTimeSeries(*S0, *S2, In, Hgauge, t1, dt1, Kgauges, Tgauges);
            PrintVariables(*S0, *S2, In, t1, dt1, Kprint, Tprint);
            S0->Update(*S2);
        }

        else if(In.OrderTime == 3) {
            if(In.IndexGauges.size() != 0) RecordTimeSeries(*S0, *S3, In, Hgauge, t1, dt1, Kgauges, Tgauges);
        ...
        if(t1>=0 && t1<=In.Time*1.5) printProgress(t1/In.Time);
        else {
    ...
    // save time series
    if(In.IndexGauges.size() != 0) PrintTimeSeries(Hgauge, In);

    std::cout << std::endl;
    std::cout << std::endl;

    std::cout << "The numerical simulation took " << end - start << " s" << std::endl;
