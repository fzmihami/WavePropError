...
WaveMaker::WaveMaker(const Input& In, const unsigned int& N)
{
    ...
    this->Freq = new float[Nf]; memset(this->Freq, 0, Nf * sizeof(float));
    this->Amp = new float[Nf]; memset(this->Amp, 0, Nf * sizeof(float));
    this->Phase = new float[Nf]; memset(this->Phase, 0, Nf * sizeof(float));

    this->Omg = new float[Nf]; memset(this->Omg, 0, Nf * sizeof(float));
    this->K = new float[Nf]; memset(this->K, 0, Nf * sizeof(float));
    this->Di = new float[Nf]; memset(this->Di, 0, Nf * sizeof(float));

    this->SourceTerm = new float[N]; memset(this->SourceTerm, 0, N * sizeof(float));
    ...
    unsigned int max_amp_index = std::max_element(this->Amp, this->Amp + Nf) - this->Amp;
    ...
    nSP = (unsigned int)std::round(Lp/ In.dx);   // the sponge layer width is the peak wavelength
	gammaSP = pow(log(1.0/epsilonSP) / log(alphaSP), 1.0/nSP); // extract correspending gamma value
	...
	for(unsigned int i=0; i<Nf; i++)
	{
		...
		Di[i] = (2*Amp[i]*(Omg[i]*Omg[i] - alpha3*G*pow(K[i],4)*pow(depth,3)))
					/
...
float wavelength(float omega, float depth)	// Compute Kn from Omega
{
	...
	if(abs(beta) >=1E-8) K = SecondorderEquation(G*beta, -(depth*omega*omega*alpha1+G), depth*omega*omega);
	else K = depth*omega*omega/(depth*omega*omega*alpha1+G);
