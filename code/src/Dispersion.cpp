...
Dispersion::Dispersion(const State &S0) {
  this->N = S0.N;
  this->PhiC = new float[N];
  memset(this->PhiC, 0, N * sizeof(float));

  if (S0.Grid == grid::staggered) {
    this->PhiM = new float[N + 1];
    memset(this->PhiM, 0, (N + 1) * sizeof(float));
    this->Uxx = new float[N + 1];
    memset(this->Uxx, 0, (N + 1) * sizeof(float));

    this->D1 = new float[N + 1];
    memset(this->D1, 0, (N + 1) * sizeof(float));
    this->D2 = new float[N + 1];
    memset(this->D2, 0, (N + 1) * sizeof(float));
    this->D3 = new float[N + 1];
    memset(this->D3, 0, (N + 1) * sizeof(float));
  } else {
    this->PhiM = new float[N];
    memset(this->PhiM, 0, N * sizeof(float));
    this->Uxx = new float[N];
    memset(this->Uxx, 0, N * sizeof(float));

    this->D1 = new float[N];
    memset(this->D1, 0, N * sizeof(float));
    this->D2 = new float[N];
    memset(this->D2, 0, N * sizeof(float));
    this->D3 = new float[N];
    memset(this->D3, 0, N * sizeof(float));
  }

  // the diagonal terms of the diseprion matrix can be precomputed
  // since they only depend on the still water level, which assumed to be
  // constant in time
  float dn = S0.depth;
  unsigned int i_end = S0.Grid == grid::staggered ? N + 1 : N;

  for (unsigned int i = 0; i < i_end; i++) {
    D2[i] = 1.0 - 2 * alpha1 * dn * dn * S0.dx_1 * S0.dx_1;
  }
  for (unsigned int i = 0; i < i_end - 1; i++) {
    D1[i] = alpha1 * dn * dn * S0.dx_1 * S0.dx_1;
  }
  for (unsigned int i = 1; i < i_end; i++) {
    D3[i] = alpha1 * dn * dn * S0.dx_1 * S0.dx_1;
  }
}
...
#pragma omp parallel for
    for (unsigned int i = 1; i < N; i++) {
      ...
      PhiC[i] = (pow(Za, 2) / 2.0f - pow(h, 2) / 6.0f) * h *
                    (Uxx[i + 1] - Uxx[i]) * S0.dx_1 +
...
#pragma omp parallel for
    for (unsigned int i = 1; i < N - 1; i++) {
      ...
      PhiC[i] = (pow(Za, 2) / 2.0f - pow(h, 2) / 6.0f) * h *
                    (Uxx[i + 1] - Uxx[i - 1]) * (0.5f * S0.dx_1) +
                (Za + h / 2.0f) * h * h * (Uxx[i + 1] - Uxx[i - 1]) *
                    (0.5f * S0.dx_1);
