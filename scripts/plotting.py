...
def create_animation_plotly(ds_list: list, ds_exact: xr.Dataset=None, frame_duration: int=200)->go.Figure:
    ...
    if ds_exact is not None:
        fig.add_trace(
            go.Scatter( x=ds_exact.x.values,
                        y=ds_exact.eta[0, :],
                        mode='lines',
    ...
    if ds_exact is not None:
        frames = [
            dict(
                ...
                data=
                    ...
                    +
                    [go.Scatter(x=ds.x.values, y=ds.eta[k, :], mode='lines') for ds in ds_list],
        ...
        frames = [
            dict(
                name=k,
                data=[go.Scatter(x=ds.x.values, y=ds.eta[k, :], mode='lines') for ds in ds_list],
    ...
    updatemenus = [dict(type='buttons',
                    buttons=[
                        dict(label='&#9658;',
                             method='animate',
                             args=[[f'{k}' for k in range(len(time))],
                                    dict(mode='immediate',
    ...
    sliders = [
        ...
        'steps': [
                ...
                'method': 'animate'} for k in range(len(time))]}]
...
def plot_gauges(ds_list: list, ds_exact: xr.Dataset=None, gauges: list=None)->go.Figure:
    ...
    -------
    fig : plotly.graph_objects.Figure
        Figure with the free surface evolution at the gauges.
    """
        
    # color palette
    colors = [
        'blue', 'red', 'green', 'cyan', 'magenta', 'yellow', 'black', 'orange'
    ]
    ...
    if gauges is not None:
        ...
        for ds in ds_list + [ds_exact]:
            new_index = np.argmin(np.abs(ds.x.values[ds.index_gauges.values][:, np.newaxis] - gauges), axis=0)
            index0.append(new_index)
            index_x.append(ds.index_gauges.values[new_index])
        
        
        # create subplot for each gauge
        subplot_title = [f'G{i+1} = {ds_list[0].x[index_x[0][i]]:.2f} m' for i in range(len(index0[0]))]
        fig = make_subplots(rows=len(index0[0]), cols=1, shared_xaxes=True, subplot_titles=subplot_title)
        ...
        for num, i in enumerate(index0[0]):
            ...
            if ds_exact is not None:
                fig.add_trace(
                    go.Scatter(x=ds_exact.time_gauges.values,
            ...
            for j, ds in enumerate(ds_list):
                fig.add_trace(
                    go.Scatter(x=ds.time_gauges.values,
                                y=ds.eta_gauges[:, i],
                                mode='lines',
    ...
    subplot_title = [f'G{i+1} = {ds_list[0].x[ds_list[0].index_gauges[i]]:.2f} m' for i in range(ds_list[0].eta_gauges.shape[1])]
    fig = make_subplots(rows=ds_list[0].eta_gauges.shape[1], cols=1, shared_xaxes=True, subplot_titles=subplot_title)
